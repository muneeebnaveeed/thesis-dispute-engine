...
#   render-realms.sh <slug> <name> <uuid>    one tenant, for onboarding
set -euo pipefail
cd "$(dirname "$0")"
...
render() {
  local slug=$1 name=$2 tenant_id=$3
...
(($# == 0)) || { echo "usage: render-realms.sh [<slug> <name> <uuid>]" >&2; exit 2; }
while IFS='|' read -r slug name tenant_id; do
  render "$slug" "$name" "$tenant_id"
done <<'TENANTS'
