...
const STATUS: Record<Deadline['status'], { label: string; tone: string }> = {
  RUNNING: { label: 'running', tone: 'bg-neutral-100 text-neutral-800' },
  MET: { label: 'met', tone: 'bg-emerald-100 text-emerald-900' },
  LATE: { label: 'met late', tone: 'bg-amber-100 text-amber-900' },
  BREACHED: { label: 'breached', tone: 'bg-red-100 text-red-900' },
  VOID: { label: 'no longer applies', tone: 'bg-neutral-100 text-neutral-500' },
}
...
export function DeadlineBadge({ status }: { status: Deadline['status'] }) {
  const s = STATUS[status]
  return <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${s.tone}`}>{s.label}</span>
}

/** Days left, or days over, as a person would say it; whole days because the deadline is an end of day. */
export function remaining(dueAt: string, now = Date.now()): string {
...
export function Deadlines({ deadlines }: { deadlines: Deadline[] }) {
