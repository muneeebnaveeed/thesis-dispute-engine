...
export function RiskBadge({ tier, score }: { tier: Tier; score?: number | undefined }) {
  return (
    <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${TONE[tier]}`}>
      {tier.toLowerCase()}
      ...
      <table className="w-full text-left" aria-label="Risk signals">
        ...
        <tbody>
          {risk.signals.map((s) => (
            <tr
              key={s.name}
              className={`border-t border-neutral-200 ${s.points === 0 ? 'text-neutral-500' : ''}`}
            >
