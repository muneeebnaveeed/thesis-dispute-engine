...
- Layout by role: `src/api/` (generated contract, `call`, failures, views), `src/queries/`
  (TanStack Query options and `useServerMutation`), `src/forms/` (form validation and input
  schemas), `src/lib/` (pure helpers: `cn`, money, email rendering), `src/components/ui/` (cva
  primitives: Button, Badge, field styles), `src/components/layout/`, `src/components/disputes/`,
  `src/components/comms/`, and `src/server/{functions,runtime,auth}/`. Components are
...
- The browser never calls the API and never holds a token (ADR 0020). Every call is a server
  function in `src/server/functions/`, built from `analystGet` or `analystPost`
  ...
  one where the contract has it) before the handler runs; `read(context.api, api => ...)` turns
...
- Style: const arrow functions everywhere (oxlint `func-style` rejects `function` declarations;
  ...
  `export const Route` at the bottom of a route file.
