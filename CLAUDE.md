...
- `make ci`: everything the `ci` workflow runs (versions, api:generate-check, api:fmt, api:vet, api:lint,
  api:test, api:tidy, workbench:check). Run
  before every push. Tests are split by build tag: `make api:test` is unit only (in-memory store,
  real handlers, no database, no network); `make api:test-integration` compiles the
  `//go:build integration` files and runs them against `make db:up` (CI uses a service
  container), failing loudly if the database is absent. `make api:cover` prints the coverage
  table CI posts on every PR.
- `make api:generate`: regenerate sqlc queries and the OpenAPI server; commit the output.
...
- `make db:up`: PostgreSQL in Docker. `make stack:up`: Postgres + the API image. `make stack:dev`:
...
- `make api:image`: build `backend/Dockerfile` locally.
...
docs/adr, docs/thesis                    decisions; the thesis outline, figure register, evaluation protocol and runs (make thesis:eval)
...
- **Spec first:** any HTTP change starts in `docs/api/openapi.yaml`, then `make api:generate`,
...
- **No C compiler locally:** `go test -race` fails with "requires cgo"; use `make api:test` and
