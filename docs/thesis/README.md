...
`make thesis:build` builds `latex/build/main.pdf` with TeX Live in Docker (no local TeX needed);
...
- Without a premium account: upload `latex/` as a zip to a new project for the language pass,
  and apply the comments here. The PDF for review is `make thesis:build` either way.
...
| `evaluation.md` | What is measured, how, and where the numbers land (`make thesis:eval`) |
