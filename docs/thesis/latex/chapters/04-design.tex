...
The proposal ended with a fraud-scoring engine, and the supervisor named it as the first thing
to cut if time ran short. It survived in a deliberately modest form, and the form is the point.
...
% To write: ADR 0008. Stub so the communications section's reference resolves.
...
trusted. An email is an entry in an outbox: a dispatcher claims unsent rows across tenants through
owner-defined functions, the same device the purge uses to cross tenant boundaries
(section~\ref{sec:design-tenancy}), renders text and HTML from the same paragraphs, hands them to
the relay, and records the outcome. A nudge after every commit keeps latency low; a timer keeps
nothing lost; a backoff that grows per attempt and row locks that skip what another replica holds
let several instances share the work. The trade is the usual one for an outbox: delivery is
at-least-once, so a relay that accepted a message but failed to say so may receive it twice,
whereas a letter, being a row, cannot be duplicated.
...
every paragraph ever sent, to which address, by which channel, and whether it went. The browser
suite closes the loop by opening a dispute and finding the acknowledgement in the development mail
relay, and by opening a Regulation~E dispute and printing its letter.
